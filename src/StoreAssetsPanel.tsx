      ...
      {view === 'screenshots' ? (
        <section id="store-screenshots" className="store-view mt-5">
          <div className="store-grid-2 grid gap-5 xl:grid-cols-2">
            ...
            <div className="store-card store-settings rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="setting-row mt-4"><span className="text-sm">Canvas background</span><div className="pill-group mt-2 flex flex-wrap items-start gap-2">{['auto', '#ffffff', '#000000'].map((value) => <button key={value} type="button" className={`pill rounded-full border px-3 py-1.5 text-xs transition ${background === value ? 'active border-neutral-950 bg-neutral-950 text-white' : 'border-neutral-300 bg-white text-neutral-700 hover:border-neutral-950'}`} onClick={() => { setBackground(value); invalidateScreenshots() }}>{value === 'auto' ? 'Auto' : value === '#ffffff' ? 'White' : 'Black'}</button>)}<ColorPicker id="store-canvas-background-color" label="Custom color" value={background} fallback="#f1f5f9" presets={[{ label: 'Soft', value: '#f4f4f2' }, { label: 'White', value: '#ffffff' }, { label: 'Black', value: '#000000' }]} onChange={(value) => { setBackground(value); invalidateScreenshots() }} /></div></div>
        ...
        <section id="store-icon" className="store-view mt-5">
          <div className="store-grid-2 grid gap-5 xl:grid-cols-2">
            ...
            <div className="store-card store-settings rounded-2xl border border-slate-200 bg-slate-50 p-4"><h4 className="font-semibold text-slate-950">Icon settings</h4><div className="field-grid mt-4"><label className="grid gap-1 text-sm font-medium text-slate-700"><span className="text-xs text-slate-500">Layout</span><select value={iconFitMode} onChange={(e: any) => { setIconFitMode(e.currentTarget.value); invalidateIcons() }}><option value="fit">Fit</option><option value="fill">Fill</option></select></label></div><div className="setting-row mt-4"><span className="text-sm">Background</span><div className="pill-group mt-2 flex flex-wrap items-start gap-2">{['transparent', '#ffffff', '#000000'].map((value) => <button key={value} type="button" className={`pill rounded-full border px-3 py-1.5 text-xs transition ${iconBackground === value ? 'active border-neutral-950 bg-neutral-950 text-white' : 'border-neutral-300 bg-white text-neutral-700 hover:border-neutral-950'}`} onClick={() => { setIconBackground(value); invalidateIcons() }}>{value === 'transparent' ? 'Transparent' : value === '#ffffff' ? 'White' : 'Black'}</button>)}<ColorPicker id="store-icon-background-color" label="Custom color" value={iconBackground} fallback="#ffffff" presets={[{ label: 'Soft', value: '#f4f4f2' }, { label: 'White', value: '#ffffff' }, { label: 'Black', value: '#000000' }]} onChange={(value) => { setIconBackground(value); invalidateIcons() }} /></div></div><div className="store-note mt-4 rounded-xl bg-white p-3 text-sm text-slate-600"><strong className="text-slate-800">Exports</strong><p>Master 1024 × 1024, Google Play 512 × 512, plus common Apple asset-catalog icon sizes. Rounded corners are preview-only.</p></div><button id="store-generate-icons" className="primary-button store-action mt-5 min-h-11 rounded-xl bg-neutral-950 px-4 py-2 font-semibold text-white transition hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-50" type="button" disabled={iconBusy} onClick={() => void generateIcons()}>{iconBusy ? 'Generating…' : 'Generate app icons'}</button></div>
          </div>
