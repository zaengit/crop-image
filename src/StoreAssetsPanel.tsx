  ...
  return (
    <article className="store-result-card overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
      ...
      <div className="store-result-body space-y-2 p-4">
        ...
        <button type="button" className="secondary-button compact min-h-9 rounded-lg border border-neutral-300 bg-white px-3 py-1.5 text-xs text-neutral-700 transition hover:border-neutral-950 hover:text-neutral-950" onClick={() => downloadBlob(item.blob, item.filename)}>Download</button>
      </div>
    </article>
  )
}
...
export function StoreAssetsPanel() {
  ...
  return (
    <section id="store-panel" className="menu-panel store-panel rounded-[28px] border border-neutral-200 bg-white p-5 shadow-[0_20px_55px_rgba(15,23,42,0.06)] sm:p-6 [&_select]:w-full [&_select]:rounded-xl [&_select]:border [&_select]:border-neutral-300 [&_select]:bg-white [&_select]:px-3 [&_select]:py-2 [&_select]:text-sm [&_select]:text-neutral-900 [&_select]:outline-none [&_select:focus]:border-neutral-950 [&_select:focus]:ring-2 [&_select:focus]:ring-neutral-200">
      ...
      <nav className="store-tabs mt-5 flex gap-2" aria-label="App Store asset type">
        <button type="button" className={`store-tab min-h-11 rounded-xl px-4 py-2 text-sm transition ${view === 'screenshots' ? 'active bg-neutral-950 font-semibold text-white shadow-sm' : 'text-neutral-600 hover:bg-neutral-100 hover:text-neutral-950'}`} aria-pressed={view === 'screenshots'} onClick={() => setView('screenshots')}>Screenshots</button>
        <button type="button" className={`store-tab min-h-11 rounded-xl px-4 py-2 text-sm transition ${view === 'icon' ? 'active bg-neutral-950 font-semibold text-white shadow-sm' : 'text-neutral-600 hover:bg-neutral-100 hover:text-neutral-950'}`} aria-pressed={view === 'icon'} onClick={() => setView('icon')}>App icon</button>
      </nav>
      ...
      {view === 'screenshots' ? (
        <section id="store-screenshots" className="store-view mt-5">
          <div className="store-grid-2 grid gap-5 xl:grid-cols-2">
              ...
              <div id="store-screenshot-list" className="source-list mt-4 space-y-2">
                {sources.map((source, index) => (
                  <article key={source.id} draggable className="source-row flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-2" onDragStart={() => { draggedId.current = source.id }} onDragOver={(event: any) => event.preventDefault()} onDrop={() => dropOnSource(source.id)}>
                    ...
                    <div className="source-actions flex gap-1 [&_button]:min-h-9 [&_button]:rounded-lg [&_button]:border [&_button]:border-neutral-300 [&_button]:bg-white [&_button]:px-2 [&_button]:py-1 [&_button]:text-xs [&_button]:font-medium [&_button]:text-neutral-700 [&_button]:transition [&_button:hover]:border-neutral-950 [&_button:hover]:text-neutral-950 [&_button:disabled]:cursor-not-allowed [&_button:disabled]:opacity-40"><button type="button" disabled={index === 0} onClick={() => moveSource(source.id, -1)}>↑</button><button type="button" disabled={index === sources.length - 1} onClick={() => moveSource(source.id, 1)}>↓</button><button type="button" onClick={() => removeSource(source.id)}>Delete</button></div>
                  </article>
                ))}
              </div>
            </div>
            ...
            <div className="store-card store-settings rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="setting-row mt-4"><span className="text-sm">Canvas background</span><div className="pill-group mt-2 flex flex-wrap gap-2">{['auto', '#ffffff', '#000000'].map((value) => <button key={value} type="button" className={`pill rounded-full border px-3 py-1.5 text-xs transition ${background === value ? 'active border-neutral-950 bg-neutral-950 text-white' : 'border-neutral-300 bg-white text-neutral-700 hover:border-neutral-950'}`} onClick={() => { setBackground(value); invalidateScreenshots() }}>{value === 'auto' ? 'Auto' : value === '#ffffff' ? 'White' : 'Black'}</button>)}<label className="color-pill rounded-full border border-neutral-300 bg-white px-3 py-1.5 text-xs text-neutral-700">Custom <input type="color" value={background.startsWith('#') ? background : '#f1f5f9'} onChange={(e: any) => { setBackground(e.currentTarget.value); invalidateScreenshots() }} /></label></div></div>
              ...
              <button id="store-generate-screenshots" className="primary-button store-action mt-5 min-h-11 rounded-xl bg-neutral-950 px-4 py-2 font-semibold text-white transition hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-50" type="button" disabled={screenshotBusy} onClick={() => void generateScreenshots()}>{screenshotBusy ? 'Generating…' : 'Generate screenshot assets'}</button>
          ...
          <div className="store-output-head mt-6 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-end"><div><h4 className="font-semibold text-slate-950">Generated screenshots</h4><p className="text-sm text-slate-600">Quality warnings are based on how much the source must be upscaled.</p></div><button id="store-download-screenshots" className="secondary-button compact min-h-10 rounded-lg border border-neutral-300 bg-white px-3 py-2 text-xs text-neutral-700 transition hover:border-neutral-950 hover:text-neutral-950 disabled:cursor-not-allowed disabled:opacity-40" type="button" disabled={!outputs.length} onClick={() => void downloadScreenshots()}>Download screenshots ZIP</button></div>
        ...
        <section id="store-icon" className="store-view mt-5">
          <div className="store-grid-2 grid gap-5 xl:grid-cols-2">
            <div className="store-card rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="store-card-head flex items-start justify-between gap-3"><div><h4 className="font-semibold text-slate-950">Source icon</h4><p className="text-sm text-slate-600">Use a square image at 1024 × 1024 or larger for the best result.</p></div><button id="store-pick-icon" className="secondary-button compact min-h-10 rounded-lg border border-neutral-300 bg-white px-3 py-2 text-xs text-neutral-700 transition hover:border-neutral-950 hover:text-neutral-950" type="button" onClick={() => iconInputRef.current?.click()}>Choose icon</button></div>
            ...
            <div className="store-card store-settings rounded-2xl border border-slate-200 bg-slate-50 p-4"><h4 className="font-semibold text-slate-950">Icon settings</h4><div className="field-grid mt-4"><label className="grid gap-1 text-sm font-medium text-slate-700"><span className="text-xs text-slate-500">Layout</span><select value={iconFitMode} onChange={(e: any) => { setIconFitMode(e.currentTarget.value); invalidateIcons() }}><option value="fit">Fit</option><option value="fill">Fill</option></select></label></div><div className="setting-row mt-4"><span className="text-sm">Background</span><div className="pill-group mt-2 flex flex-wrap gap-2">{['transparent', '#ffffff', '#000000'].map((value) => <button key={value} type="button" className={`pill rounded-full border px-3 py-1.5 text-xs transition ${iconBackground === value ? 'active border-neutral-950 bg-neutral-950 text-white' : 'border-neutral-300 bg-white text-neutral-700 hover:border-neutral-950'}`} onClick={() => { setIconBackground(value); invalidateIcons() }}>{value === 'transparent' ? 'Transparent' : value === '#ffffff' ? 'White' : 'Black'}</button>)}<label className="color-pill rounded-full border border-neutral-300 bg-white px-3 py-1.5 text-xs text-neutral-700">Custom <input type="color" value={iconBackground.startsWith('#') ? iconBackground : '#ffffff'} onChange={(e: any) => { setIconBackground(e.currentTarget.value); invalidateIcons() }} /></label></div></div><div className="store-note mt-4 rounded-xl bg-white p-3 text-sm text-slate-600"><strong className="text-slate-800">Exports</strong><p>Master 1024 × 1024, Google Play 512 × 512, plus common Apple asset-catalog icon sizes. Rounded corners are preview-only.</p></div><button id="store-generate-icons" className="primary-button store-action mt-5 min-h-11 rounded-xl bg-neutral-950 px-4 py-2 font-semibold text-white transition hover:bg-neutral-800 disabled:cursor-not-allowed disabled:opacity-50" type="button" disabled={iconBusy} onClick={() => void generateIcons()}>{iconBusy ? 'Generating…' : 'Generate app icons'}</button></div>
          </div>
          <div className="store-output-head mt-6 flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-end"><div><h4 className="font-semibold text-slate-950">Generated app icons</h4><p className="text-sm text-slate-600">No rounded corners are baked into exported files.</p></div><button id="store-download-icons" className="secondary-button compact min-h-10 rounded-lg border border-neutral-300 bg-white px-3 py-2 text-xs text-neutral-700 transition hover:border-neutral-950 hover:text-neutral-950 disabled:cursor-not-allowed disabled:opacity-40" type="button" disabled={!iconOutputs.length} onClick={() => void downloadIcons()}>Download icons ZIP</button></div>
