...
function ResultCard({ item, icon = false }: { item: StoreOutput | IconOutput; icon?: boolean }) {
  ...
  return (
    <article className="store-result-card overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/60">
      <div className={`${icon ? 'icon-result-preview' : ''} store-result-preview flex min-h-44 items-center justify-center bg-slate-900 p-3`}>
      ...
      <div className="store-result-body space-y-2 p-4">
        ...
        <small className="block text-slate-400">{item.width} × {item.height}</small>
        {quality ? <small className={`block ${upscale > 2 ? 'quality-low text-rose-300' : upscale > 1.5 ? 'quality-warn text-amber-300' : 'quality-good text-emerald-300'}`}>{quality}</small> : null}
        <button type="button" className="secondary compact rounded-lg border border-slate-700 px-3 py-1.5 text-xs" onClick={() => downloadBlob(item.blob, item.filename)}>Download</button>
      </div>
    </article>
  )
}
...
export function StoreAssetsPanel() {
  ...
  return (
    <section id="store-panel" className="menu-panel store-panel rounded-3xl border border-slate-800 bg-slate-900/60 p-5">
      <div className="panel-heading flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div><h3 className="text-xl font-semibold text-white">App Store assets</h3><p className="mt-1 text-sm text-slate-400">Create screenshots, feature graphics, and app icons for Google Play and Apple App Store.</p></div>
      </div>
      <nav className="store-tabs mt-5 flex gap-2" aria-label="App Store asset type">
        <button type="button" className={`store-tab rounded-xl px-4 py-2 text-sm ${view === 'screenshots' ? 'active bg-cyan-500 font-semibold text-slate-950' : 'border border-slate-700'}`} aria-pressed={view === 'screenshots'} onClick={() => setView('screenshots')}>Screenshots</button>
        <button type="button" className={`store-tab rounded-xl px-4 py-2 text-sm ${view === 'icon' ? 'active bg-cyan-500 font-semibold text-slate-950' : 'border border-slate-700'}`} aria-pressed={view === 'icon'} onClick={() => setView('icon')}>App icon</button>
      </nav>

      {view === 'screenshots' ? (
        <section id="store-screenshots" className="store-view mt-5">
          <div className="store-grid-2 grid gap-5 xl:grid-cols-2">
            <div className="store-card rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
              <div className="store-card-head flex items-start justify-between gap-3">
                <div><h4 className="font-semibold text-white">Screenshots</h4><p className="text-sm text-slate-400">Upload up to 10 screenshots. Drag to reorder before generating.</p></div>
                <button id="store-pick-screenshots" className="secondary compact rounded-lg border border-slate-700 px-3 py-2 text-xs" type="button" onClick={() => screenshotInputRef.current?.click()}>Add screenshots</button>
              </div>
              ...
              <div id="store-screenshot-drop" className="mini-dropzone mt-4 rounded-xl border border-dashed border-slate-700 p-5 text-center text-sm text-slate-500" onDragOver={(event: any) => event.preventDefault()} onDrop={(event: any) => { event.preventDefault(); if (event.dataTransfer?.files) void addScreenshotFiles(event.dataTransfer.files) }}>Drop screenshots here</div>
              <div id="store-screenshot-list" className="source-list mt-4 space-y-2">
                {sources.map((source, index) => (
                  <article key={source.id} draggable className="source-row flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-900 p-2" onDragStart={() => { draggedId.current = source.id }} onDragOver={(event: any) => event.preventDefault()} onDrop={() => dropOnSource(source.id)}>
                    ...
                    <div className="source-actions flex gap-1"><button type="button" disabled={index === 0} onClick={() => moveSource(source.id, -1)}>↑</button><button type="button" disabled={index === sources.length - 1} onClick={() => moveSource(source.id, 1)}>↓</button><button type="button" onClick={() => removeSource(source.id)}>Delete</button></div>
                  </article>
                ))}
              </div>
            </div>

            <div className="store-card store-settings rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
              <h4 className="font-semibold text-white">Output settings</h4>
              <div className="field-grid mt-4 grid gap-3 sm:grid-cols-2">
                <label><span>Platform</span><select value={platform} onChange={(e: any) => { setPlatform(e.currentTarget.value); invalidateScreenshots() }}><option value="both">Google Play + Apple App Store</option><option value="google">Google Play</option><option value="apple">Apple App Store</option></select></label>
                <label><span>Orientation</span><select value={orientation} onChange={(e: any) => { setOrientation(e.currentTarget.value); invalidateScreenshots() }}><option value="portrait">Portrait</option><option value="landscape">Landscape</option><option value="both">Both</option></select></label>
                <label><span>Resize mode</span><select value={resizeMode} onChange={(e: any) => { setResizeMode(e.currentTarget.value); invalidateScreenshots() }}><option value="fit">Fit</option><option value="smart">Smart crop</option><option value="fill">Fill</option></select></label>
                <label><span>Format</span><select value={format} onChange={(e: any) => { setFormat(e.currentTarget.value); invalidateScreenshots() }}><option value="png">PNG</option><option value="jpeg">JPEG</option></select></label>
              </div>
              <div className="setting-row mt-4"><span className="text-sm">Canvas background</span><div className="pill-group mt-2 flex flex-wrap gap-2">{['auto', '#ffffff', '#000000'].map((value) => <button key={value} type="button" className={`pill rounded-full border px-3 py-1.5 text-xs ${background === value ? 'active border-cyan-400 text-cyan-200' : 'border-slate-700'}`} onClick={() => { setBackground(value); invalidateScreenshots() }}>{value === 'auto' ? 'Auto' : value === '#ffffff' ? 'White' : 'Black'}</button>)}<label className="color-pill rounded-full border border-slate-700 px-3 py-1.5 text-xs">Custom <input type="color" value={background.startsWith('#') ? background : '#f1f5f9'} onChange={(e: any) => { setBackground(e.currentTarget.value); invalidateScreenshots() }} /></label></div></div>
              ...
              <button id="store-generate-screenshots" className="primary store-action mt-5 rounded-xl bg-cyan-500 px-4 py-2 font-semibold text-slate-950 disabled:opacity-50" type="button" disabled={screenshotBusy} onClick={() => void generateScreenshots()}>{screenshotBusy ? 'Generating…' : 'Generate screenshot assets'}</button>
              <small id="store-screenshot-status" className="store-status mt-3 block text-sm text-slate-400" aria-live="polite">{screenshotStatus}</small>
            </div>
          </div>
          <div className="store-output-head mt-6 flex items-end justify-between gap-3"><div><h4 className="font-semibold text-white">Generated screenshots</h4><p className="text-sm text-slate-400">Quality warnings are based on how much the source must be upscaled.</p></div><button id="store-download-screenshots" className="secondary compact rounded-lg border border-slate-700 px-3 py-2 text-xs disabled:opacity-40" type="button" disabled={!outputs.length} onClick={() => void downloadScreenshots()}>Download screenshots ZIP</button></div>
        ...
        <section id="store-icon" className="store-view mt-5">
          <div className="store-grid-2 grid gap-5 xl:grid-cols-2">
            <div className="store-card rounded-2xl border border-slate-800 bg-slate-950/40 p-4">
              <div className="store-card-head flex items-start justify-between gap-3"><div><h4 className="font-semibold text-white">Source icon</h4><p className="text-sm text-slate-400">Use a square image at 1024 × 1024 or larger for the best result.</p></div><button id="store-pick-icon" className="secondary compact rounded-lg border border-slate-700 px-3 py-2 text-xs" type="button" onClick={() => iconInputRef.current?.click()}>Choose icon</button></div>
              ...
              <div id="store-icon-drop" className="mini-dropzone mt-4 rounded-xl border border-dashed border-slate-700 p-5 text-center text-sm text-slate-500" onDragOver={(event: any) => event.preventDefault()} onDrop={(event: any) => { event.preventDefault(); const file = event.dataTransfer?.files?.[0]; if (file) void setIconFile(file) }}>Drop an icon here</div>
              <small id="store-icon-source-info" className="store-status mt-3 block text-sm text-slate-400">{iconStatus}</small>
              {iconSource ? <div className="icon-preview-pair mt-4 grid grid-cols-2 gap-4"><div><span className="text-xs text-slate-500">Square</span><div className="icon-preview mt-2 aspect-square overflow-hidden border border-slate-800" style={{ background: iconBackground === 'transparent' ? 'transparent' : iconBackground }}><img src={iconSource.url} alt="App icon preview" className={`h-full w-full ${iconFitMode === 'fit' ? 'object-contain' : 'object-cover'}`} /></div></div><div><span className="text-xs text-slate-500">Rounded preview</span><div className="icon-preview rounded mt-2 aspect-square overflow-hidden rounded-[24%] border border-slate-800" style={{ background: iconBackground === 'transparent' ? 'transparent' : iconBackground }}><img src={iconSource.url} alt="Rounded app icon preview" className={`h-full w-full ${iconFitMode === 'fit' ? 'object-contain' : 'object-cover'}`} /></div></div></div> : null}
            </div>
            <div className="store-card store-settings rounded-2xl border border-slate-800 bg-slate-950/40 p-4"><h4 className="font-semibold text-white">Icon settings</h4><div className="field-grid mt-4"><label><span>Layout</span><select value={iconFitMode} onChange={(e: any) => { setIconFitMode(e.currentTarget.value); invalidateIcons() }}><option value="fit">Fit</option><option value="fill">Fill</option></select></label></div><div className="setting-row mt-4"><span className="text-sm">Background</span><div className="pill-group mt-2 flex flex-wrap gap-2">{['transparent', '#ffffff', '#000000'].map((value) => <button key={value} type="button" className={`pill rounded-full border px-3 py-1.5 text-xs ${iconBackground === value ? 'active border-cyan-400 text-cyan-200' : 'border-slate-700'}`} onClick={() => { setIconBackground(value); invalidateIcons() }}>{value === 'transparent' ? 'Transparent' : value === '#ffffff' ? 'White' : 'Black'}</button>)}<label className="color-pill rounded-full border border-slate-700 px-3 py-1.5 text-xs">Custom <input type="color" value={iconBackground.startsWith('#') ? iconBackground : '#ffffff'} onChange={(e: any) => { setIconBackground(e.currentTarget.value); invalidateIcons() }} /></label></div></div><div className="store-note mt-4 rounded-xl bg-slate-900 p-3 text-sm text-slate-400"><strong className="text-slate-200">Exports</strong><p>Master 1024 × 1024, Google Play 512 × 512, plus common Apple asset-catalog icon sizes. Rounded corners are preview-only.</p></div><button id="store-generate-icons" className="primary store-action mt-5 rounded-xl bg-cyan-500 px-4 py-2 font-semibold text-slate-950 disabled:opacity-50" type="button" disabled={iconBusy} onClick={() => void generateIcons()}>{iconBusy ? 'Generating…' : 'Generate app icons'}</button></div>
          </div>
          <div className="store-output-head mt-6 flex items-end justify-between gap-3"><div><h4 className="font-semibold text-white">Generated app icons</h4><p className="text-sm text-slate-400">No rounded corners are baked into exported files.</p></div><button id="store-download-icons" className="secondary compact rounded-lg border border-slate-700 px-3 py-2 text-xs disabled:opacity-40" type="button" disabled={!iconOutputs.length} onClick={() => void downloadIcons()}>Download icons ZIP</button></div>
