...
type EnhancementMode = 'auto' | 'manual' | null
    ...
    <section id="enhance-global" className="enhance-global rounded-[28px] border border-neutral-200 bg-white p-5 shadow-[0_20px_55px_rgba(15,23,42,0.06)] backdrop-blur sm:p-6">
      <div className="enhance-head flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-neutral-600">Optional. Choose Auto for one-click enhancement or Manual for full control.</p>
        </div>
        <div className="enhance-actions flex flex-wrap gap-2">
          ...
          <button id="enhance-reset" className="secondary-button min-h-11 rounded-xl border border-neutral-300 bg-white px-4 py-2 text-sm font-medium text-neutral-700 transition hover:border-neutral-950 hover:text-neutral-950" type="button" onClick={resetEnhancement}>Reset</button>
        </div>
      </div>

      <div className="mt-6 grid gap-3 sm:grid-cols-2" role="group" aria-label="Image quality enhancement mode">
        <button
          id="enhance-auto"
          type="button"
          aria-pressed={mode === 'auto'}
          className={`rounded-2xl border p-4 text-left transition ${mode === 'auto' ? 'border-neutral-950 bg-neutral-950 text-white' : 'border-neutral-200 bg-neutral-50 text-neutral-950 hover:border-neutral-950'}`}
          onClick={selectAuto}
        >
          <span className="block text-sm font-semibold">Auto</span>
          <span className={`mt-1 block text-xs leading-5 ${mode === 'auto' ? 'text-neutral-300' : 'text-neutral-500'}`}>Automatically improve the image with one click.</span>
        </button>
        <button
          id="enhance-manual"
          type="button"
          aria-pressed={mode === 'manual'}
          className={`rounded-2xl border p-4 text-left transition ${mode === 'manual' ? 'border-neutral-950 bg-neutral-950 text-white' : 'border-neutral-200 bg-neutral-50 text-neutral-950 hover:border-neutral-950'}`}
          onClick={selectManual}
        >
          <span className="block text-sm font-semibold">Manual</span>
          <span className={`mt-1 block text-xs leading-5 ${mode === 'manual' ? 'text-neutral-300' : 'text-neutral-500'}`}>Adjust light, color, detail, and AI enhancement tools yourself.</span>
        </button>
      </div>
