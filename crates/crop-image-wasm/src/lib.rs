...
fn face_group_focus(regions: &[FocusRegion], padding: f32) -> Option<Point> {
    let faces: Vec<&FocusRegion> = regions.iter().filter(|r| r.kind == "face" && r.confidence >= 0.5).collect();
    ...
    for r in faces {
        ...
        max_x = max_x.max(r.x + r.width); max_y = max_y.max(r.y + r.height);
    }
    let pad_x = (max_x - min_x) * padding.max(0.0);
    let pad_y = (max_y - min_y) * padding.max(0.0);
    min_x = (min_x - pad_x).clamp(0.0, 1.0); max_x = (max_x + pad_x).clamp(0.0, 1.0);
    min_y = (min_y - pad_y).clamp(0.0, 1.0); max_y = (max_y + pad_y).clamp(0.0, 1.0);
    Some(Point { x: ((min_x + max_x) * 0.5).clamp(0.0, 1.0), y: ((min_y + max_y) * 0.47).clamp(0.0, 1.0) })
}
...
fn saliency_focus(rgba: &[u8], width: u32, height: u32) -> Point {
    ...
    for sy in 1..sample_h.saturating_sub(1) {
        for sx in 1..sample_w.saturating_sub(1) {
            ...
            let w = detail * center_bias; weighted_x += nx * w; weighted_y += ny * w; weight_sum += w;
        }
    }
    if weight_sum <= f64::EPSILON { Point { x: 0.5, y: 0.5 } } else { Point { x: (weighted_x / weight_sum) as f32, y: (weighted_y / weight_sum) as f32 } }
}

fn apply_safe_area(focus: Point, safe_top: f32, safe_bottom: f32) -> Point {
    let top = safe_top.clamp(0.0, 0.45); let bottom = safe_bottom.clamp(0.0, 0.45);
...
pub fn smart_crop_png(
    rgba: &[u8], src_width: u32, src_height: u32, target_width: u32, target_height: u32,
    focus_regions_json: &str, safe_top: f32, safe_bottom: f32, face_padding: f32,
    manual_x: f32, manual_y: f32,
    ...
    let base_focus = if manual_x >= 0.0 && manual_y >= 0.0 {
        Point { x: manual_x.clamp(0.0, 1.0), y: manual_y.clamp(0.0, 1.0) }
    } else {
        face_group_focus(&regions, face_padding).unwrap_or_else(|| saliency_focus(rgba, src_width, src_height))
    };
...
mod tests {
    use super::*;
    #[test] fn landscape_to_portrait_keeps_full_height() { let (_, _, w, h) = crop_rect(4000, 3000, 1080, 1350, Point { x: 0.5, y: 0.5 }); assert_eq!(h, 3000); assert_eq!(w, 2400); }
    #[test] fn focus_moves_crop_without_overflowing() { let (x, _, w, _) = crop_rect(4000, 3000, 1080, 1350, Point { x: 0.95, y: 0.5 }); assert_eq!(w, 2400); assert_eq!(x, 1600); }
    #[test] fn multiple_faces_focus_between_people() { let faces = vec![FocusRegion { x: 0.1, y: 0.2, width: 0.2, height: 0.3, confidence: 0.9, kind: "face".into() }, FocusRegion { x: 0.7, y: 0.2, width: 0.2, height: 0.3, confidence: 0.95, kind: "face".into() }]; let focus = face_group_focus(&faces, 0.1).unwrap(); assert!((focus.x - 0.5).abs() < 0.02); }
    #[test] fn safe_area_clamps_vertical_focus() { let focus = apply_safe_area(Point { x: 0.5, y: 0.98 }, 0.1, 0.2); assert!(focus.y <= 0.8); }
    #[test] fn manual_focus_can_target_edge() { let (x, _, _, _) = crop_rect(4000, 3000, 1080, 1350, Point { x: 0.0, y: 0.5 }); assert_eq!(x, 0); }
}
